...
- **Sweeps** fassen viele kurze Faelle unter einem Namen zusammen und fahren sie zu je
  ...
  Belagswechsel und Hindernisse waere sonst nicht in vertretbarer Zeit zu fahren (307 Faelle
...
- `config.yml` ist menschenlesbar (km/h, m/s², %, °/s, Grad, Sekunden, Bloecke); `CarConfig.reload()` konvertiert in Blöcke/Tick UND clampt jeden Wert über `clampHumanValue` auf dessen Sinn-Bereich — genau denselben wirksamen Wert zeigt `/car config` an (ein Display-RoHWert-Bug zeigte sonst abweichende Zahlen). Neue Keys müssen in `CarConfig.NUMBER_KEYS`/`BOOL_KEYS`/`STRING_KEYS` (Komplettierung + Migration lesen daraus).
- **Ein neuer Zahlen-Key braucht vier Eintraege, sonst faellt der Selftest:** Default in der ausgelieferten `config.yml` (`config-keys`), Obergrenze in `clampHumanValue` (`config-obergrenzen`), Einheit in `SelfTest.UNITS` und Zweig in `SelfTest.configValue` (`config-einheiten`). Dazu die Einheiten-Endung in `CarCommand.unitSuffix` und — bei String-Keys — der Zweig in `CarCommand.stringValue`. Cooldowns stehen in **Sekunden** (`Unit.SEKUNDEN`, x20), Drehraten in **Grad pro Sekunde** (`Unit.PRO_SEKUNDE`, /20).
- **Deckel und Schwellen der Physik gehoeren in `PINNED_CONFIG`,** sobald sie aus der Config kommen: sonst kippt eine von Hand verstellte `config.yml` auf dem Testserver die Szenarien, gegen die sie kalibriert sind (seit 1.5.1 betrifft das u. a. `crash-rebound-max`, `water-drag`, `standstill-speed`).
...
- **`/car reload`** ist der Gegenpol zu `reset`: statt der JAR-Defaults liest es genau das ein, was gerade auf der Platte steht (`plugin.reloadConfig()` + `carConfig.reload()`) — fuer Hand-Edits an der `config.yml` am laufenden Server, ohne Neustart. Teilt sich die `car.config.*`-Node mit dem vollen Reset (beide koennen jeden Fahrwert auf einen Schlag aendern). Der Selftest-Fall `config-reload` schreibt dafuer direkt auf die Platte (an `plugin.getConfig()` vorbei, `YamlConfiguration.save`) und prueft, dass der Aufruf das uebernimmt — mit Restore der Originaldatei im `finally`, sonst faerbt ein Testlauf den Server dauerhaft ein.
- **Keine stillen Korrekturen:** `CarConfig.reload()` liest Zahlen ueber `number()` und Schalter ueber `bool()` statt direkt ueber `getDouble`/`getBoolean`. Der Grund ist Bukkits Verhalten: ein Wert vom falschen Typ (`max-speed: schnell`) faellt kommentarlos auf den Default zurueck, und ein Wert ausserhalb des Sinnbereichs wurde frueher kommentarlos geklemmt — der Admin sah in beiden Faellen NICHTS und suchte stundenlang, warum seine Einstellung nicht wirkt. Dazu meldet `reportUnknownKeys()` Tippfehler im Key-Namen (die sonst wirkungslos in der Datei stehen). Jede Korrektur geht ins Log (`config.yml — <key>: …`) UND wird in `lastCorrections` gesammelt; `/car reload` gibt sie zusaetzlich an den Aufrufer aus. Wer eine neue Leseart hinzufuegt, nimmt `number()`/`bool()` — sonst faellt der Wert wieder still zurueck.
- **`/car reload` parst die Datei ZUERST selbst** (`new YamlConfiguration().load(file)`), bevor `plugin.reloadConfig()` laeuft. Grund: Bukkit schluckt einen Syntaxfehler — es loggt ihn nur und liefert eine **leere** Konfiguration. Ohne die Vorpruefung meldet der Befehl gruen "neu eingelesen", waehrend in Wahrheit jeder Fahrwert auf dem Default steht; das naechste `/car config <key> <wert>` schriebe das per `saveConfig()` in die Datei und die kaputte, aber reparierbare Konfiguration waere endgueltig weg. Beim Serverstart schuetzt das Backup nach `config.veraltet.yml` davor, bei `reload` gibt es keins. `config-reload-kaputt` sichert das ab (ohne den Guard faellt der Fall rot — gegengeprueft).
- **Umbenannte Keys brauchen einen Eintrag in `AutoPlugin.RENAMED_KEYS`** (alter → neuer Name), sonst fällt der Wert bei der Migration weg und der Nutzer steht wieder auf dem Default. So gewandert: `understeer-sound` → `understeer-sound-enabled` (1.5.0). **Ein frei gewordener Name darf spaeter neu vergeben werden** — seit 1.5.1 ist `understeer-sound` der Sound-NAME. Damit der alte Schalter nicht als Sound-Name "false" in der neuen Datei landet, prueft `carryOver` zusaetzlich den TYP des alten Wertes (Zahl/Boolean/String) und ueberspringt ihn sonst; `config-migration-typen` sichert das ab.
- `config-version` im YAML: bei Änderung hochzählen (`AutoPlugin.CONFIG_VERSION`, aktuell 15); die Migration sichert automatisch als `config.veraltet.yml` und übernimmt unveränderte Keys. **Geänderte Default-WERTE erreichen einen bestehenden Server nicht** — `carryOver` übernimmt jeden gemeinsamen Key aus der alten Datei, auch bei Versionssprung. Wer die neuen Defaults will, löscht seine `config.yml` (das gilt auch für `.testserver/plugins/Auto/config.yml`).
...
- Untersteuer-Sound: Default `minecraft:entity.horse.death` mit Pitch 0 (gewollt so — erst ganz unten klingt das Wiehern nach Reifen), Reichweite 8 Bloecke, Cooldown 0,9 s über `tickCount` in `DriveTask` — tick-basiert statt Wall-Clock, lag-stabil. Seit 1.5.1 stehen Sound, Pitch, Reichweite, Cooldown und Schwelle (`understeer-min-slip`, Schraeglaufwinkel) in der Config; der Schalter bleibt `understeer-sound-enabled` (Default aus).
